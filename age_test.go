...
func TestAgeOnFails(t *testing.T) {
	...
	expected := errors.New("age on: date of birth is after provided date")

	got, err := person.AgeOn(testDOB, date, format)
	if err == nil {
		t.Fatalf("AgeOn(%s, %s, %s) = %s, want to fail with error: %s", testDOB.Format(time.RFC3339),
			date.Format(time.RFC3339), format, got, expected.Error())
	} else {
